...
export function TravelsContextProvider({
  ...
  const LoadUserTravels = async (
    ...
    let travelsPageData: TravelsPageData = { travels: [], totalPages: 1 };
    try {
      const baseUrl = `/travels/user/${userData.id}?`;
      ...
      if (res.data) {
        travelsPageData.travels = res.data.content.map(
          (entry: TravelResponse) => {
            return TravelResponseToTravelData(entry);
          }
        );
        travelsPageData.totalPages = res.data.totalPages;
      }
    } catch (err: any) {
      addMessage(err, PopupMessageType.ERROR, 1000);
    }
    return travelsPageData;
  };
