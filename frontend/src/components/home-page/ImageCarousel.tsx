...
function ImageCarousel() {
  ...
  return (
    <div className="relative mx-auto xl:ml-auto flex items-center justify-center h-96 mb-10 sm:mb-0  w-[90%] sm:w-full max-w-xl">
      <motion.ul
        className={`absolute w-[40%] h-full left-[50%] -translate-x-[50%]`}
      ...
      >
        {indexes.map((idx, i) => (
          <motion.li
            layout
            key={i}
            className="absolute origin-center w-full"
            initial={{ rotate: 0, x: 0, y: 0, opacity: 1 }}
            animate={{
              rotate: (idx - 1) * 10,
              x: (idx - Math.floor(images.length / 2)) * 100,
              y: idx === 0 ? 50 : idx * -10,
